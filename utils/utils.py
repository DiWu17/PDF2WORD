...
def parse_pdfs(
    ...
    :param output_dir: 存储解析结果的输出目录。
    :param lang: 语言选项，默认为'ch'。
    :param backend: 解析后端，默认为'pipeline'。
    :param method: 解析方法，默认为'auto'。
    :param server_url: 'vlm-sglang-client'后端的服务器URL。
    :param start_page_id: 解析的起始页面ID。
    :param end_page_id: 解析的结束页面ID。
    """
    try:
        file_name_list = []
        pdf_bytes_list = []
...
def parse_pdf(
    ...
    :param end_page_id: 解析的结束页面ID。
    """
    parse_pdfs([path], output_dir, lang, backend, method, server_url, start_page_id, end_page_id)
